...
<Leader><CR> | 設定reload 
<Leader>o | 現在のbuffer以外を閉じる
<Leader>w | 保存
<Leader>q | 終了
<Leader>h | 左へbuffer移動
<Leader>l | 右へbuffer移動
<Leader>x | 現在のbufferを削除
...
<C-h> | 左へウィンドウ移動
<C-i> | iはo(Older cursur position)の左, ジャンプリストの中の[count]だけ新しいカーソル位置に移動
<C-j> | 下へウィンドウ移動
<C-k> | 上へウィンドウ移動
<C-l> | 右へウィンドウ移動
...
<C-u> | Extra lines, 上へ半ページウィンドウをスクロール
<C-v> | 矩形Visualモードへ
<C-y> | 上へ[count]行ウィンドウをスクロール
<C-w>o | 現在のWindow以外閉じる
